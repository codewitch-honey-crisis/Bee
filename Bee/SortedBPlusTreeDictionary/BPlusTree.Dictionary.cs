...
namespace Bee
{
		...
		System.Collections.IEnumerable
	{
		...
		#region BPlusTreeReverseEnumerator
		private struct ReverseEnumerator : IEnumerator<KeyValuePair<TKey, TValue>>
		{
			// Long form is 10% faster than yield syntax form.
			SortedBPlusTreeDictionary<TKey, TValue> target;
			LeafNode currentLeaf;
			...
			/// containing these key/value pairs.
			/// </param>
			public ReverseEnumerator(SortedBPlusTreeDictionary<TKey, TValue> dictionary)
			{
				currentLeaf = null;
				leafIndex = 0;
				_version = 0;
			...
			public bool MoveNext()
			{
				...
				leafIndex = currentLeaf.KeyCount - 1;
				return currentLeaf != null;
			...
			{
				_version = target.VersionDirect;
				currentLeaf = target.GetLastLeaf();
				if (null != currentLeaf)
					leafIndex = currentLeaf.KeyCount;
				else
					leafIndex = 0;
